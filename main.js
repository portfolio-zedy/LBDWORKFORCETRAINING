...
let user, allProgress = [], pendingReviews = [], pendingRetakes = [], allUsers = [], allCourses = [], milestones = [], progress = [];
...
function statusBadge(status) {
  const map = {
    ...
    RETAKE_PENDING: ["⏳", "badge--submitted", "Retake Requested"],
  };
...
async function doLogin() {
  ...
  try {
    ...
    const result = await apiCall("login", { username, email, password_hash });
...
async function doSignup() {
  ...
  try {
    const password_hash = await sha256(password);
    const result = await apiCall("signup", { fname, lname, username, email, password_hash, tribe_id, hub_id, colony, phone, start_date });
...
async function doFacilitatorSignup() {
  ...
  try {
    const password_hash = await sha256(password);
    const result = await apiCall("signupFacilitator", { name, username, email, password_hash, created_at });
...
function renderAdminCoursesTable() {
  ...
  tbody.innerHTML = allCourses.map(c => `
    <tr>
      <td style="font-weight:500;color:var(--text-main)">${c.title}</td>
...
async function loadPendingReviews() {
  const [revRes, retRes] = await Promise.all([
    apiCall("getPendingReviews", { requester_role: user.role }),
    apiCall("getPendingRetakes", { requester_role: user.role })
  ]);
  
  pendingReviews = revRes.pending || [];
  pendingRetakes = retRes.pending || [];
...
function updatePendingBadge() {
  ...
  if(!badge) return;
  const count = pendingReviews.length + pendingRetakes.length;
  if (count > 0) { 
      badge.style.display = "inline"; 
      badge.textContent = count; 
      badge.style.marginLeft = "auto"; badge.style.background = "var(--error)"; badge.style.color = "white";
      badge.style.borderRadius = "10px"; badge.style.padding = "0.1rem 0.5rem"; badge.style.fontSize = "0.7rem";
  } else { 
...
function renderRetakesTable() {
  const tbody = document.getElementById("retakeBody");
  if(!tbody) return;
  if (!pendingRetakes.length) { tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--text-dim);padding:2rem">No retake requests pending. 🎉</td></tr>`; return; }
  
  tbody.innerHTML = pendingRetakes.map(req => `
    <tr id="retake-row-${req.milestone_id}-${req.user_id}">
      <td>${req.full_name || req.user_id}</td>
      <td style="color:var(--text-muted)">${req.milestone_title || req.milestone_id}</td>
      <td style="font-family:'JetBrains Mono',monospace;color:var(--error);">${req.score || 0}%</td>
      <td>
          <button style="padding:0.4rem 0.875rem;background:var(--success-bg);color:var(--success);border:1px solid rgba(34,197,94,0.25);border-radius:6px;font-size:0.8rem;font-weight:500;cursor:pointer;font-family:'Inter',sans-serif;" onclick="approveRetake('${req.user_id}','${req.milestone_id}','${req.course_id}')">🔓 Approve Retake</button>
      </td>
    </tr>`).join("");
}

async function approveRetake(userId, milestoneId, courseId) {
  const row = document.getElementById(`retake-row-${milestoneId}-${userId}`);
  if (row) row.style.opacity = "0.5";
  const res = await apiCall("approveRetake", { user_id: userId, milestone_id: milestoneId, course_id: courseId, requester_role: user.role });
  
  if (res.success) {
    showToast("Retake approved. Module is unlocked for the member.", "success");
...
function renderStats() {
  ...
  const enrolledMembers = allUsers.filter(u => u.enrolled_courses && u.enrolled_courses.trim() !== "").length;
  const pending = pendingReviews.length + pendingRetakes.length;
  const grouped = groupProgressByMember(allProgress);
  const completedMembers = grouped.filter(m => m.total > 0 && m.rate === 100).length;

  el.innerHTML = `
    <div class="stat-card"><div style="font-family:'DM Serif Display',serif;font-size:2rem;color:var(--gold-lt);line-height:1;margin-bottom:4px;">${totalMembers}</div><div style="font-size:0.78rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.07em;">Members</div></div>
    <div class="stat-card"><div style="font-family:'DM Serif Display',serif;font-size:2rem;color:var(--gold-lt);line-height:1;margin-bottom:4px;">${enrolledMembers}</div><div style="font-size:0.78rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.07em;">Enrolled Members</div></div>
    <div class="stat-card"><div style="font-family:'DM Serif Display',serif;font-size:2rem;color:var(--gold-lt);line-height:1;margin-bottom:4px;">${pending}</div><div style="font-size:0.78rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.07em;">Pending Actions</div></div>
    <div class="stat-card"><div style="font-family:'DM Serif Display',serif;font-size:2rem;color:var(--gold-lt);line-height:1;margin-bottom:4px;">${completedMembers}</div><div style="font-size:0.78rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.07em;">Members Completed</div></div>`;
}
...
function renderProgressTable(data) {
  ...
  tbody.innerHTML = grouped.map(m => `
    <tr>
      <td><button onclick="openMemberDetail('${m.user_id}')" style="background:none;border:none;padding:0;color:var(--gold-lt);font-weight:600;font-size:.875rem;cursor:pointer;font-family:inherit;text-decoration:underline;text-underline-offset:3px;">${m.full_name || m.user_id}</button></td>
      ...
      <td style="font-family:'JetBrains Mono',monospace;font-size:.85rem;color:var(--gold-lt)">${m.rate}%</td>
      ...
      <div class="learner-card" data-name="${(data.name||"").toLowerCase()}">
        ...
        <div>
          ${data.milestones.sort((a,b)=>(a.order_index||0)-(b.order_index||0)).map(m => {
            ...
            if(m.status === "SUBMITTED" || m.status === "RETAKE_PENDING") dotColor = "var(--warning)";
...
function switchView(name) {
  ...
  const viewTitles = {
    ...
    pending:  ["Pending Actions", "Submissions and retake requests"],
...
function closeMilestoneModal() { document.getElementById("milestoneModal").style.display = "none"; }
...
function openAssignModal(uid, name) {
  ...
  sel.innerHTML = '<option value="">— Select course —</option>' + allCourses.map(c => `<option value="${c.course_id}">${c.title}</option>`).join("");
...
async function createUser() {
  ...
  const password_hash = await sha256(password);
  const res = await apiCall("createUser", { full_name: name, username, password_hash, email, role: "LEARNER", course_id: course, requester_role: user.role });
  if (res.success) {
...
function renderCourseList() {
  ...
  el.innerHTML = userRoadmap.map((c) => {
    ...
    return `<button class="course-btn ${currentCourse?.course_id === c.course_id ? 'active' : ''}" style="${lockedStyle}" onclick="${clickAction}">
      <span style="font-size: 14px;">${icon}</span> ${c.title}
    </button>`;
  }).join("");
}
  ...
  userRoadmap.forEach((c) => {
    ...
    html += `
      <div class="roadmap-card ${cardClass}" onclick="${c.is_unlocked ? `openCourse('${c.course_id}')` : ''}">
        ...
        <div class="roadmap-title">${c.title}</div>
        <div class="roadmap-desc">${c.description}</div>
  ...
  const html = `
    <div style="margin-bottom: 2rem; display: flex; align-items: flex-start; justify-content: space-between; gap: 1rem;">
      <div>
        ...
        <h2 style="font-family: 'DM Serif Display', serif; font-size: 1.6rem; color: var(--text-main); margin-bottom: 0.4rem;">${currentCourse.title}</h2>
        <p style="font-size: 0.875rem; color: var(--text-muted);">${currentCourse.description}</p>
      </div>
    </div>
...
function renderMilestoneCard(m, idx) {
  ...
  if(status === "SUBMITTED" || status === "RETAKE_PENDING") statusStyle = "background: var(--warning-bg); color: var(--warning); border: 2px solid var(--warning);";
  ...
  return `
    <div class="milestone-card" id="card-${m.milestone_id}" style="background: var(--bg-panel); border: 1px solid; ${borderStyle} border-radius: 14px; margin-bottom: 1rem; overflow: hidden; transition: border-color 0.2s; ${locked ? 'opacity: 0.55;' : ''}">
      <div style="padding: 1.25rem 1.5rem; display: flex; align-items: center; gap: 1rem; ${locked ? 'cursor: default;' : 'cursor: pointer;'}" onclick="${!locked?`toggleCard('${m.milestone_id}')`:''}">
        ...
        <div style="flex: 1;">
          <div style="font-weight: 600; font-size: 0.95rem; color: var(--text-main); margin-bottom: 3px;">${m.title}</div>
          <div style="font-size: 0.8rem; color: var(--text-dim);">${m.description}</div>
        </div>
      ...
      <div id="body-${m.milestone_id}" style="display: none; padding: 0 1.5rem 1.5rem; border-top: 1px solid var(--border-subtle);">
        ...
        <div style="background: var(--bg-surface); border-radius: 10px; padding: 1.25rem 1.5rem; margin: 1rem 0; font-size: 0.875rem; line-height: 1.75; color: var(--text-muted);">${m.content_html}</div>
...
function renderSubmissionArea(m, status) {
  ...
  if (status === "RETAKE_PENDING") return `<div style="background: var(--warning-bg); border: 1px solid rgba(245,158,11,0.25); border-radius: 10px; padding: 1rem 1.25rem; font-size: 0.875rem; color: var(--warning); display: flex; align-items: center; gap: 8px;">⏳ Retake request sent. Please wait for a facilitator to unlock this assessment.</div>`;
  
  if (status === "FAILED") {
    const p = progress.find(p => p.milestone_id === m.milestone_id);
    return `
      <div style="background: var(--error-bg); border: 1px solid rgba(239,68,68,0.25); border-radius: 10px; padding: 1.5rem; text-align: center;">
        <div style="font-size: 2rem; margin-bottom: 0.5rem; color: var(--error); font-family: 'DM Serif Display', serif;">${p?.score || 0}%</div>
        <p style="color: var(--error); font-size: 0.9rem; margin-bottom: 1rem; font-weight: 500;">❌ Not Passed. You must score at least ${m.pass_score}%.</p>
        <button class="btn-action btn-secondary" onclick="requestRetake('${m.milestone_id}', '${m.course_id}')" style="margin: 0 auto; border-color: var(--error); color: var(--error);">Request Retake</button>
      </div>`;
  }
...
async function requestRetake(mid, courseId) {
  showToast("Sending request...", "info");
  const res = await apiCall("requestRetake", { user_id: user.user_id, milestone_id: mid, course_id: courseId });
  if (res.success) {
    showToast("Retake requested successfully!", "success");
...
function buildSubmissionForm(m) {
  ...
  if (type === "CHECKBOX") {
    ...
    html += `Quiz Assessment (pass mark: ${m.pass_score}%)</div><div id="quiz-${m.milestone_id}">`;
    (quiz.questions || []).forEach((q, qi) => {
      html += `<div id="qq-${m.milestone_id}-${q.id}" style="background: var(--bg-surface); border-radius: 10px; padding: 1.25rem; margin-bottom: 1rem;">
    ...
    html += `</div><button class="btn-action btn-primary" id="btn-${m.milestone_id}" onclick="submitQuiz('${m.milestone_id}', '${currentCourse.course_id}')">Submit Quiz</button><div id="quiz-result-${m.milestone_id}" style="display:none"></div>`;
...
async function submitQuiz(mid, courseId) {
  ...
  if (res.success) {
    ...
    if (!res.passed) resultEl.innerHTML += `<button class="btn-action btn-secondary" style="margin-top: 0.75rem; padding: 0.625rem 1.25rem; font-size: 0.875rem;" onclick="refreshProgress('${courseId}')">Close</button>`;
