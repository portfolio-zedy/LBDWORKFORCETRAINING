...
function renderStats() {
  ...
  // Total active learners found directly on the sheet (via allUsers list)
  const totalMembers = allUsers.length;

  // Enrolled Members (Active learners assigned to at least one course)
  ...
  // Completed Members (Learners who have finished all their assigned milestones with a 100% completion rate)
  ...
  el.innerHTML = `
    ...
    <div class="stat-card"><div style="font-family:'DM Serif Display',serif;font-size:2rem;color:var(--gold-lt);line-height:1;margin-bottom:4px;">${pending}</div><div style="font-size:0.78rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.07em;">Awaiting Review</div></div>
...
async function loadCoursesLearner() {
  ...
  allCourses = res.courses || [];
  renderCourseList();
  if (allCourses.length === 1) selectCourse(allCourses[0]);
}

function renderCourseList() {
  const el = document.getElementById("courseList");
  if(!el) return;
  if (!allCourses.length) { el.innerHTML = `<p style="font-size:.8rem;color:var(--text-dim);padding:.5rem 0">No paths enrolled.</p>`; return; }
  el.innerHTML = allCourses.map((c,i) => `<button class="course-btn ${currentCourse?.course_id===c.course_id?'active':''}" onclick="selectCourse(allCourses[${i}])"><span style="font-size: 14px;">📋</span> ${c.title}</button>`).join("");
}
...
async function selectCourse(course) {
  ...
  renderCourseList();
  closeMobileSidebar();
  if(document.getElementById("topbarTitle")) document.getElementById("topbarTitle").textContent = course.title;
  if(document.getElementById("topbarSub")) document.getElementById("topbarSub").textContent = course.description;
  const tc = document.getElementById("trackContainer");
  if(tc) tc.innerHTML = `<div class="skeleton" style="height:70px;margin-bottom:1rem;border-radius:12px;"></div><div class="skeleton" style="height:80px;margin-bottom:1rem;border-radius:14px;"></div><div class="skeleton" style="height:80px;border-radius:14px;"></div>`;
  const [mRes, pRes] = await Promise.all([ apiCall("getMilestones", { course_id: course.course_id }), apiCall("getProgress", { user_id: user.user_id, course_id: course.course_id }) ]);
...
function renderVideoEmbed(url, mid, needsGate) {
  ...
  } else { return `<div style="margin-top:1rem;"><a href="${vinfo.trimmed}" target="_blank" rel="noopener" class="btn-action btn-secondary" style="padding:0.6rem 1rem;font-size:0.85rem;">▶ Watch video</a></div>`; }
...
async function submitQuiz(mid, courseId) {
  ...
  if (res.success) {
    ...
    if (!res.passed) resultEl.innerHTML += `<button class="btn-action btn-secondary" style="margin-top: 0.75rem; padding: 0.625rem 1.25rem; font-size: 0.875rem;" onclick="refreshProgress('${courseId}')">Try Again</button>`;
...
async function loadCoursesLearner() {
  ...
  // Fetch the new dynamic dashboard data
  ...
  let html = `
    <div class="welcome-dashboard-wrap">
      <div class="welcome-dashboard-content">
        <div style="margin-bottom: 2rem;">
          <h2 style="font-family: 'DM Serif Display', serif; font-size: 2rem; color: var(--text-main); margin-bottom: 0.4rem;">Welcome, ${user.full_name.split(' ')[0]}</h2>
